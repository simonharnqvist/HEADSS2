...
def hdbscan(df: pd.DataFrame,
            min_cluster_size:int, min_samples:int, allow_single_cluster:bool, 
            cluster_method: str, cluster_columns:"list[str]", drop_upgrouped:bool = True) -> pd.DataFrame:
    ...
    df.loc[:, 'group'] = clusterer.labels_ + int(df.loc['region'])
...
def cluster(region_partitioned_split_data: dd.DataFrame, min_cluster_size:int, min_samples:int, allow_single_cluster:bool, 
    ...
    return region_partitioned_split_data.map_partitions(hdbscan, 
